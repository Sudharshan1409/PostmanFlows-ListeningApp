...
@app.route('/microsoft/webhookhandler', methods=['GET', 'POST'])
def microsoft_webhook_handler():
    if 'validationToken' in request.args:
        return Response(request.args.get('validationToken'))

    print("Request in microsoft handler", request.json)
    # notification = request.json

    return Response('Notification received', status=200)
